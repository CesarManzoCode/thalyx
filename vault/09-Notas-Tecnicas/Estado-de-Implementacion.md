...
| Motor de edición de texto | `crates/thalyx-edit` | Direcciones por renglón **y sustitución de una cadena exacta en varios archivos en una llamada**, guardado atómico, deshacer acotado. Conserva finales de renglón, salto final y permisos; un enlace se edita como el archivo al que apunta. La sustitución precomprueba todos los archivos antes de escribir uno, rechaza un salto de línea en cualquiera de los dos textos —así nunca cambia cuántos renglones tiene un archivo— y dice `wrote: false` en cada rechazo |
