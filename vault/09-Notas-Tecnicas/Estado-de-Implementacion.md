...
| Frontera de plataforma | `crates/thalyx-platform`, `crates/thalyx-cli/src/platform.rs` | **Desde el 2026-09-12 `hacer` no llama a ningún mecanismo de Linux directamente**: todo lo que su semántica le pide a la máquina pasa por `Platform` —estado versionado, lanzamiento, trabajo, evidencia, reloj, transporte y autoridad como tipos—, y la semántica se quedó en `exec.rs`. Dos backends, elegidos con `THALYX_PLATFORM`: `linux-current`, que es exactamente el mecanismo Btrfs de antes y el de omisión, y `linux-managed`. Una traza común (`thalyx-trace-v1`) en la evidencia, nunca en la respuesta. Ver [[Frontera-de-Plataforma]] |
| Modelo administrado en Linux | `crates/thalyx-managed`, `thalyx_platform::managed` | `managed-local-v1` de Thalyx-Kernel sobre archivos: un escritor, objetos por digest, log encadenado con `fsync`, `PREPARE`/`COMMIT` con recibo, CAS sobre la generación, secuencias por principal, recuperación que aborta lo preparado, obra privada, candidato congelado del que trata cada veredicto, cerca de trabajo. Diez pruebas del store (incluidos los dos cortes y la vista que quedó atrás) y cinco de `hacer` sobre él con columna de control. Su perfil declara `holds: false`: el escritor exclusivo se **detecta** por digest, no se previene. Ver [[Frontera-de-Plataforma]] |
| Corpus de equivalencia EXP-13 | `dev/exp13/`, `crates/thalyx-cli/tests/exp13_equivalence.rs` | Dieciséis casos de sólo entradas, conducidos por un agente con guion sobre el socket del bridge. `linux-current` contra el binario de `0492f72` sin modificar, exacto; `linux-managed` contra `linux-current`, en significado, con las diferencias que cada caso declara. Etapa **62** de `verify.sh` |
