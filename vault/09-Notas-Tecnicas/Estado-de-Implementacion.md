...
| `llama.cpp` como proceso | `crates/thalyx-agent/llama.rs` | Invoca `llama-completion`, no `llama-cli`. Plazo, tope de salida, muestreo de `VmHWM`, y **comprobación del contrato de una pasada**. Una inferencia real **sí terminó**: 6.88 s y 4.77 GB medidos. `grammar-check` separa gramática aplicada de bandera aceptada, y falta correrlo; ninguna gama medida aún |
