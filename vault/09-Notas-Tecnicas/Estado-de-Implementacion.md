...
| El intento con nombre | `crates/thalyx-core/attempt.rs`, verbo `intento` | Política probada contra el falso de directorios; el Btrfs lo ejerce la etapa 26. Desde el 2026-08-29 `abandonar` se puede pedir en **una** llamada nombrando el intento (`snapshot=`) y declarando lo que cuesta (`delete=`, `revert=`): procede sólo si el nombre es el del registro y los dos números son exactamente lo que el árbol tiene, así que trabajo ajeno aparecido mientras el intento estaba abierto lo detiene en vez de perderse. La decisión entera es una función pura, `consent`, con siete pruebas que corren sin Btrfs. La forma de dos pasos queda intacta |
...
| Métricas de una sesión de agente | `crates/thalyx-mcp/src/metrics.rs` | Tiempo, llamadas, herramientas, bytes enviados **y** devueltos, errores, archivos leídos, búsquedas, intentos. Sin tokens: este proceso no los ve, y una estimación sería un número que parece medición. Desde el 2026-08-29 también `machine_requests` y `machine_seconds` — preguntas que de verdad llegaron a la máquina, que no son las llamadas, y cuánto tardaron en contestarse: es lo único que separa el puente del tiempo que el modelo pasa pensando, y deja que cada corrida real conteste eso sobre sí misma sin gastar nada |
