...
| Sesión del sistema | `crates/thalyx-cli/session.rs` | Lo que init arranca; solo dice que es la máquina cuando lo es. `disponibles`, `instalar <id>`, `permisos`, `revertir`, `modulos`, `correr <id>`, `estado`, `nucleo`, `apagar` — adentro no hay shell, así que lo que no es un verbo de aquí no existe. Cubre los pasos 2, 3, 4 y 5 del [[Criterio-de-Salida-Fase-1]] |
