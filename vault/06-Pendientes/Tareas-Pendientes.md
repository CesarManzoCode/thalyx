...
- [ ] **Correr `nucleo` en hierro y ver qué hizo el USB.** Es la medición que el arreglo de la consola vuelve posible: el buffer conserva las líneas de nivel informativo de la enumeración que la consola nunca imprimió, y son las que dicen si faltó una opción o si el dongle es lento. Hasta entonces la causa del `-110` **no está determinada**.
