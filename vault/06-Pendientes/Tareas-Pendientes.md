...
- [ ] **Correr `make -C image run-hardware`.** Es el acto 2 hasta donde una máquina alcanza: xHCI con teclado USB, un NVMe y un AHCI en blanco, y la imagen instalada colgada como memoria USB. **No cierra el acto 2** — no hay silicio real ni una memoria física — pero sí responde que los cuatro grupos de controladores compilan, enlazan y producen los dispositivos con los nombres correctos. Nunca ha corrido en ningún sitio. Ver [[Criterio-de-Salida-Fase-1]].
