...
- [ ] **Correr una gama de verdad.** El camino real de `llama.cpp` está construido y **no ha corrido nunca**: el contenedor no tiene el binario ni alcanza los pesos. Lo que falta es bajar un GGUF de Qwen2.5, `thalyx agent model use <gama> --weights <archivo>`, y `thalyx agent model check`. Eso responde de una vez las dos cosas que no se pueden responder aquí: que ese `llama.cpp` acepte las banderas y que tome la gramática. Después, `thalyx agent bench`. Ver [[Gamas-de-Modelo]] y [[Punto-Actual]].
