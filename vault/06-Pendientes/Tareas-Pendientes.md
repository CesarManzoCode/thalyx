...
- [ ] **Los tres subvolúmenes, desde dentro de la imagen.** Un sistema de archivos recién escrito no tiene ninguno y PID 1 monta `subvol=system`, así que un store hecho por `thalyx disk format` todavía no es un store — y el comando lo dice al terminar. No hay binario `btrfs` adentro, así que van por ioctl. Con eso el instalador queda completo. Ver [[Construccion-del-ISO]].
