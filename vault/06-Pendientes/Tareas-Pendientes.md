...
- [ ] **Confirmar las gamas con el banco** — [[Gamas-de-Modelo]] decreta cuatro gamas de una familia como hipótesis de partida. **El banco existe desde el 2026-08-08** (`thalyx agent bench`) y **ninguna gama se ha corrido**: falta una máquina con `llama.cpp` y los pesos, que es la suya. Lo que se sustituye con la corrida son los tamaños estimados por los medidos —`thalyx agent model use` ya escribe los bytes reales— y la tabla de acierto por gama, que hoy no existe. Ver [[Punto-Actual]].
