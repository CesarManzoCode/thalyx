...
- [ ] **Saber qué es `usb 1-6` en la máquina de Cesar.** Un dispositivo USB cuyo descriptor no se puede leer (`error -110`, `ETIMEDOUT`) y que el kernel reintenta para siempre, dejando el prompt inusable. Se contesta desde Fedora con `lsusb -t` y `dmesg | grep -i "1-6"`, **sin riesgo y sin arrancar Thalyx**. Hasta entonces no se agrega ninguna opción de kernel: sería adivinar, y todavía no está descartado que sea un dispositivo defectuoso y no un hueco de la configuración.
- [ ] **Decidir cómo sobrevive la sesión al ruido del kernel.** `init.rs:393` ya había previsto el síntoma —*«an info-level message arriving mid-line steps on it»*— y eligió el umbral por gravedad, que no distingue un error que ocurre una vez de uno que se repite sin parar. Además `set_console_loglevel(4)` suprime las advertencias mientras `is_trouble()` las cuenta como problema, así que las dos mitades del criterio no coinciden. Es decisión de Cesar: bajar el umbral, darle a la sesión su propia terminal virtual, o que Thalyx hable de sí mismo en vez de dejar pasar el flujo del kernel. Ver [[Punto-Actual]].
- [ ] **Correr `discos` en hierro.** No se alcanzó. La máquina tiene un `sda` además del `sdb` de la memoria, así que hay otro disco SCSI —probablemente SATA por AHCI— y `discos` es lo que lo diría, junto con el NVMe real. Ver [[Arranque-en-Hierro]].
