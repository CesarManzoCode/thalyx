...
- [ ] **Averiguar por qué el arranque tarda ~40 segundos.** `sd ... [sdb]` a los 38 s y `struct module` a los 34 s, **iguales desde dos memorias distintas** — un tiempo constante entre medios distintos es un plazo fijo, no una lectura lenta. `nucleo lento` es el instrumento y está construido; falta correrlo. **La causa no está determinada y no se va a adivinar.**
