...
- [ ] **De dónde saca el instalador el kernel cuando corre dentro de la máquina.** Hoy `thalyx install` lo recibe por `--kernel`, que es correcto para una persona con una terminal y no alcanza para el criterio de salida: adentro de la máquina arrancada desde la ISO no hay ruta que nombrar. Hace falta un **lector** de FAT y una forma de decir cuál de los discos es el medio del que se arrancó — la respuesta probable tiene la misma forma que la etiqueta del store: buscar una partición de tipo ESP con `\EFI\BOOT\BOOTX64.EFI` adentro, y **negarse si hay dos** en vez de elegir. Y un verbo de sesión que lo invoque, porque adentro no hay shell. Ver [[Construccion-del-ISO]].
...
- [ ] **Los controladores de una PC de verdad.** `thalyx.config` sale de `allnoconfig` más lo que QEMU necesita. Faltan consola sobre el framebuffer, teclado USB (xHCI y HID) y almacenamiento real (NVMe, AHCI). Cada uno es una opción de Kconfig, y **es la única parte del criterio de salida que una VM no puede responder.** Ver [[Construccion-del-ISO]].
