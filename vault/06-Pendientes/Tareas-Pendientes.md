...
- [ ] **Cerrar el acto 2b instalando en una segunda memoria USB.** Cesar tiene tres (4, 8 y 32 GB): arrancar de la A, `instalar-en` la B, quitar la A y encender. Firmware real, una GPT real escrita en un disco físico real, y un arranque real desde ese disco sin medio puesto. Lo único que no responde es que el disco sea interno, y **si eso cumple el decreto lo decide Cesar**, porque el decreto es suyo. Ver [[Criterio-de-Salida-Fase-1]].
...
- [ ] **Saber por qué el `-110` no volvió, que no es lo mismo que saber que se arregló.** No consta si el receptor Telink estaba conectado en esa segunda corrida. Si lo estaba, el fallo es **intermitente** y sigue vivo; si no lo estaba, quedó identificado por eliminación. Son dos conclusiones distintas y sólo una pregunta las separa. **Ninguna opción de kernel se agregó y ninguna se ha justificado todavía.**
