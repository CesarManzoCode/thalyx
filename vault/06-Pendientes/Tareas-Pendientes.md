...
- [ ] **Terminar una inferencia de verdad.** El camino real se **arrancó** por primera vez el 2026-08-08 contra `llama.cpp b1-3653e6d` y Qwen2.5-3B, y falló: Thalyx pedía `llama-cli`, que desde que `llama.cpp` partió sus herramientas es el frontend de chat y abre una sesión en vez de completar. Corregido —el binario por omisión es `llama-completion` y el contrato se comprueba— pero **ninguna inferencia ha terminado nunca contra pesos reales**, así que la gramática aceptada, la respuesta después del marcador y el acierto por gama siguen sin probarse. Falta `thalyx agent model check` y después `thalyx agent bench`. Ver [[Gamas-de-Modelo]] y [[Punto-Actual]].
...
- [ ] **Confirmar las gamas con el banco** — [[Gamas-de-Modelo]] decreta cuatro gamas de una familia como hipótesis de partida. **La gama media se corrió el 2026-08-08**: 6/9 intención, 3/4 abstención, 4.78 GB de RSS contra ~8 GB estimados, mediana 6.58 s. Faltan las otras tres, que son otros tres GGUF en su máquina. La estimación de RAM iba alta por casi el doble en la única gama medida, así que la tabla del decreto sigue siendo hipótesis en tres de cuatro filas. Ver [[Punto-Actual]].
- [ ] **Averiguar por qué se abstuvo con el id dicho en claro** — el banco de la gama media dejó pasar `dev.thalyx.demo, ese`, que es su caso más fácil. **La hipótesis que se había anotado aquí —que la instrucción de abstención del prompt pesa de más— quedó retirada el mismo día**: el banco contaba todo error como abstención correcta, así que ese `MISS` pudo ser una abstención real o un fallo disfrazado. La suite ahora tiene casos que varían sólo el verbo y sólo el contexto alrededor de ese mismo id, así que una corrida contesta cuál es. **No se toca el prompt antes de esa corrida.** Ver [[Gamas-de-Modelo]].
- [ ] **Decidir qué hacer con un módulo mencionado y luego descartado** — el único caso del banco en que la gama media **actuó sobre algo que la persona había excluido**. No inventó un id: tomó uno que estaba en la conversación y había sido rechazado. La negación es un problema de comprensión, no de gramática, y no se arregla restringiendo la salida. La suite tiene ahora tres formas de negación en vez de una, porque un caso no puede distinguir una gama que maneja mal la negación de una que manejó mal esa frase. Falta decidir si es cosa del prompt, del ensamblado del transcript, o algo que simplemente se le pide a la gama alta. Ver [[Gamas-de-Modelo]].
