...
- [ ] **Quién crea el store en una máquina que no tiene uno.** `store_disk.rs` decreta que PID 1 **monta y nunca fabrica**, porque una máquina que se inventa un store arranca perfecta el día que el disco no estaba. La respuesta probable conserva el decreto entero: **quien crea el store es el instalador**, que es un acto humano explícito, y PID 1 sigue sin fabricar nada. Falta confirmarlo al construirlo, y que **«es el primer arranque» y «no encontré el tuyo» sigan siendo hechos distintos**. Ver [[Construccion-del-ISO]].
