...
mod tests {
    ...
    /// The lockfile a real Rust workspace has committed.
    ///
    /// Byte for byte what `cargo generate-lockfile` writes for a one-package
    /// workspace, because a hand-written approximation is rewritten by the
    /// first tool that reads it — which is rule 6 in the small, and is exactly
    /// what happened to the first version of this constant.
    ...
    fn a_tree_that_has_to_be_looked_at() -> (tempfile::TempDir, Store, PathBuf, Where) {
        a_workspace(&[
            ...
            // **A lockfile, because a real workspace has one committed.**
            //
            // Found on 2026-08-30 by the program's own strict assertion —
            // `the tree shows 4 change(s) and the program made 3` — where the
            // fourth was `Cargo.lock`. rust-analyzer resolves the full
            // dependency graph, which writes a lock, and a workspace with none
            // therefore **gains a file because it was asked a question**:
            // a read that mutates the tree, inside the transaction, attributed
            // to nobody.
            //
            // Nothing is hidden by fixing the fixture — the lock still shows up
            // in `changed` when it is created, and a rollback still puts the
            // tree back without it. What the fixture is for is the *program's*
            // claim, and a workspace with no lockfile is not the case that
            // claim is about. The hazard itself is in
            // `vault/03-Primitivas/Semantica-Compilada.md`.
            ("Cargo.lock", LOCKFILE),
            (
                "src/lib.rs",
                "pub mod one;\npub mod two;\npub mod three;\npub mod four;\npub mod five;\n",
    ...
    #[test]
    fn the_program_verify_runs_is_the_one_that_is_tested_here() {
        // **One program, two places that run it.**
        //
        // `dev/verify.sh`'s stage 59 needs a real Btrfs subvolume, a kernel
        // that denies and a compiler, so it only runs on Cesar's machine. This
        // runs the *same file* against the directory-backed fake, here, on
        // every `cargo test` — so a typo in it is found by whoever wrote it
        // rather than by the person who owns the hardware.
        //
        // What is **not** proven here is what stage 59 exists for: that the
        // boundary is a real snapshot, that the confinement really denies, and
        // that the compiler really ran. Rule 3, and it is why both exist.
        let source = std::fs::read_to_string(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("../../dev/programs/looking-decides.js"),
        )
        .expect("the program dev/verify.sh runs");
        ...
        let evidence = run(
            ...
            &program(serde_json::json!({
                "label": "only what needs it",
                "run": source,
                // The compiler cannot run under confinement here, so the `rust`
                // check answers `not_proven` and the program takes its
                // `gave_up` branch. That is the honest outcome and it is
                // asserted below rather than being worked around — a fixture
                // that pretended the check had passed would be testing a
                // machine nobody has.
                "on_failure": "keep",
            })),
        );
        ...
        assert_eq!(
            ...
            Some("returned"),
            "the program did not run to the end: {}. changed: {:?}",
            evidence.reason,
            evidence.changed
        );
        assert_eq!(
        ...
        assert_eq!(evidence.change_count, 3);
        // Two files that had no reason to be touched, and were not.
        for name in ["two", "four"] {
            let text = std::fs::read_to_string(tree.join(format!("src/{name}.rs"))).expect(name);
            assert!(!text.contains("new_api"), "src/{name}.rs: {text}");
        }

        // And the branch the machine's own limits force it down, said out
        // loud: either the compiler ran and it committed, or it could not and
        // the program gave up. Never a third thing, and never a commit over a
        // check that did not run.
        assert!(
            evidence.returned["compiled"].is_boolean(),
            "{}",
            evidence.returned
        );
        if evidence.returned["compiled"] == serde_json::json!(false) {
            assert!(
                evidence.returned["gave_up"].is_string(),
                "the program said it did not compile and did not say why: {}",
                evidence.returned
            );
        }
    }
